...
- **localStorage**: ~5-10KB per cached location
- **React state**: ~50KB (weather data + UI state)
...
**Symptom**: Temperature or conditions don't match other sources

**Reality Check**:
- OpenWeatherMap uses nearest weather station (~10km radius)
- Different sources use different station networks
- ±2°C variance is normal between services
- Check coordinates shown in UI match your location
- Forecast accuracy: ~80-95% for 24h, decreases beyond that
...
- Add new interpretation rules (`src/interpretation/rules.ts`)
- Improve error messages (`src/components/ErrorBanner.tsx`)
- Add more cities to mock data (`src/data/mockWeatherData.ts`)
- Fix typos in weather interpretations
...
- Get your own API key (don't reuse someone else's)
- Free tier = 1000 calls/day (consider paid plan for production)
